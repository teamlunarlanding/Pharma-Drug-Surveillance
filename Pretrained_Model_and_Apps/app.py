...
# Consent box
consent = st.radio(
    "Welcome to the Adverse Drug Event Risk Prediction and Surveillance System. This system does not store any information from you. "
    "This system should not be used to make healthcare decisions without guidance from a healthcare professional. "
    "Please confirm your understanding.",
    ('I understand and wish to continue using the system.', 'I do not consent to use the system.')
)

if consent == 'I do not consent to use the system.':
    st.warning('You must consent to use the system.')
else:
    ...
    # Map serious_outcome to labels
    data['serious_outcome_label'] = data['serious_outcome'].map(outcome_labels)

    # Sidebar for prediction inputs
    st.sidebar.subheader('Input Patient Information for Prediction')
    age = st.sidebar.number_input('Age', min_value=0, max_value=120, value=30)
    weight = st.sidebar.number_input('Weight (kg)', min_value=0.0, max_value=500.0, value=70.0)
    unit_price = st.sidebar.number_input('Unit Price', min_value=0.0, max_value=250000.0, value=10.0)
    sex = st.sidebar.radio('Select Sex:', options=[1, 2], format_func=lambda x: 'Male' if x == 1 else 'Female')
    ...
    ndc9_code = st.sidebar.text_input('NDC-9 Drug Code: e.g., 59212-562')
    ...
    st.subheader('Top 10 Reported Drugs by Outcome')
    top_drugs = data.groupby(['serious_outcome', 'med_product']).size().reset_index(name='count')
    top_drugs = top_drugs.groupby('serious_outcome').apply(lambda x: x.nlargest(10, 'count')).reset_index(drop=True)

    plt.figure(figsize=(10, 8))
    sns.barplot(x='count', y='med_product', hue='serious_outcome', data=top_drugs, ci=None)
