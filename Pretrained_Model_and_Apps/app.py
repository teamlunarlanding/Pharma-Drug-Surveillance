...
# Find the index of the column where value = 1
column_index = ndc9_encoded.columns.get_loc(ndc9_column)

# Determine the range of columns to display (2 on the left and 2 on the right)
start_col = max(column_index - 2, 0)
end_col = min(column_index + 3, len(ndc9_encoded.columns))

# Slice the DataFrame to display only the relevant columns
ndc9_encoded_subset = ndc9_encoded.iloc[:, start_col:end_col]

# Display the selected columns for debugging
st.subheader('Selected NDC9 Columns')
st.write(ndc9_encoded_subset)

# Combine with the rest of the input data
...
# Combine with the selected NDC9 encoded columns
input_data_encoded = pd.concat([input_data, ndc9_encoded_subset], axis=1)
